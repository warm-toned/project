...
public class ChenShunDao extends DataDao {
	 ...
	 * @return
	 */
	public List<Map<String, Object>> MenuList(Integer id, Integer pageindex,
			Integer pagesize, String type, Integer price, String name,
	...
	public int AddMenu(ts_menu t) {
		...
		return update(sql, t.getMurtid(), t.getMuname(), t.getMuprice(), t
				.getMupic(), t.getMutype(), t.getMudesc(), t.getMustatus());
	 ...
	 * @return
	 */
	public List<Map<String, String>> OrderList(Integer id, Integer status,
			Integer pageindex, Integer pagesize, String sort, String order) {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		String sql = "select * from(select realname,ouuid,ortid,rtname,rtpic,to_char(odate,'yyyy-MM-dd HH24:mi:ss') as times,ostatus,row_number()over(order by to_char(odate,'yyyy-MM-dd HH24:mi:ss') desc) rm from ts_order o  inner join ts_restaurant r   on o.ortid=r.rtid   inner join ts_user u  on u.userid=o.ouserid  and ouuid is not null and ortid=?";
		...
		for (Map<String, Object> map : getMapList(sql, id, pagesize*(pageindex - 1)+1
			...
			map2.put("TIMES", map.get("TIMES").toString());
	...
	public int UpdateOrder(ts_order t) {
		...
		return update(sql, t.getOcount(), t.getOstatus(), t.getOrtid(), t
				.getOuserid(), t.getOmuid());
	...
	public Map<String, Object> getList(Integer id) throws Exception {
		...
		for (Map<String, Object> map2 : list) {
			map.put(map2.get("MUNAME").toString(), map2.get("MUPRICE")
					.toString());
	...
	public List<Map<String, String>> OrderListNo(Integer id, Integer pageindex,
			Integer pagesize, String sort, String order) { // 店的id
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		String sql = "select * from(select realname,ouuid,ortid,to_char(odate,'yyyy-MM-dd HH24:mi:ss') as times,rtname,rtpic,ostatus,row_number()over(order by to_char(odate,'yyyy-MM-dd HH24:mi:ss') desc) rm from ts_order o  inner join ts_restaurant r   on o.ortid=r.rtid   inner join ts_user u  on u.userid=o.ouserid  and ouuid is not null and ortid=? and ostatus=2   group by realname,ouuid,ortid,rtname,rtpic,to_char(odate,'yyyy-MM-dd HH24:mi:ss'),ostatus) where rm between ? and ?";
		if (sort != null && !sort.equals("")) {
			sql += " order by   " + sort + "  " + order;
		} else {
			sql += " order by times asc";
	...
	public List<Map<String, String>> OrderListYes(Integer id,
			Integer pageindex, Integer pagesize, String sort, String order) {
		List<Map<String, String>> list = new ArrayList<Map<String, String>>();
		String sql = "select * from(select realname,ouuid,ortid,to_char(odate,'yyyy-MM-dd HH24:mi:ss') as times,rtname,rtpic,ostatus,row_number()over(order by to_char(odate,'yyyy-MM-dd HH24:mi:ss') desc) rm from ts_order o  inner join ts_restaurant r   on o.ortid=r.rtid   inner join ts_user u  on u.userid=o.ouserid  and ouuid is not null and ortid=? and ostatus=3   group by realname,ouuid,ortid,rtname,rtpic,to_char(odate,'yyyy-MM-dd HH24:mi:ss'),ostatus) where rm between ?  and ? ";
		if (sort != null && !sort.equals("")) {
			sql += " order by   " + sort + "  " + order;
		}else{
			sql+="  order by times asc";
